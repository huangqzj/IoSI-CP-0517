...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    """
    count = 0
    def __init__(self, params, visualize, train=True, uni_time_delay = -1):
    ...
    def __getitem__(self, idx):
        base_data_dict = self.retrieve_base_data(idx,
                                                 cur_ego_pose_flag=self.cur_ego_pose_flag, uni_time_delay=self.uni_time_delay)
        ...
        # print('base_data_dict.items():',len(base_data_dict.items()),base_data_dict.keys())
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            selected_cav_processed = self.get_item_single_car(
                selected_cav_base,
                ego_lidar_pose)
            # print(float(selected_cav_base['time_delay']),float(selected_cav_base['time_delay'])<3)
            #if (int(cav_id) - 10000 == int(ego_id) or int(cav_id) - 10001 == int(ego_id)) == False:
            #if float(selected_cav_base['time_delay']) < 3 :
            object_stack.append(selected_cav_processed['object_bbx_center'])
            object_id_stack += selected_cav_processed['object_ids']
            # else:
                # print('selected_cav_base time_delay:',float(selected_cav_base['time_delay']) )

            processed_features.append(
                selected_cav_processed['processed_features'])  ## lidar after transformed

            velocity.append(selected_cav_processed['velocity'])
            # print('cav_id,velocity:', cav_id, velocity)
            time_delay.append(float(selected_cav_base['time_delay']))
            # print('cav_id,selected_cav_processed:', cav_id, time_delay, selected_cav_processed['object_ids'])
        ...
        # print("len(processed_features),len(object_stack),len(object_id_stack),len(velocity):",len(processed_features),len(object_stack),len(object_id_stack),len(velocity))
        ...
        # print("merged_feature_dict:",merged_feature_dict)
        ...
        # print('self.max_cav - len(spatial_correction_matrix):',self.max_cav, len(spatial_correction_matrix),self.max_cav - len(spatial_correction_matrix))
        ...
        if self.visualize:
            processed_data_dict['ego'].update({'origin_lidar':
                np.vstack(
                    projected_lidar_stack)})
        # print("processed_data_dict:",processed_data_dict)
